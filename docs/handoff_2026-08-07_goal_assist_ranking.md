...
| 레이어 | 할 일 |
|---|---|
| **FPC(콘솔)** | ✅ 완료 — 골·어시스트를 식별 가능한 형태로 보내도록 수정 (§3). **단 배포 전** |
| **FinePlay 백엔드** | ⬜ **이 문서의 작업** — 저장된 클립 액션에서 집계 (§4) |
| **FinePlay 앱** | — **수정 없음.** 기존 `goalProduction`/`assistProduction`을 그대로 읽습니다 |

> ⚠️ **선행 조건**: 이 스펙은 콘솔이 `baseAction`·`Goal`·`Assist`를 실어 보내기 시작한 뒤에야 값이 채워집니다. 콘솔 변경은 아직 **머지·배포 전**입니다(작성 시점 기준 PR 리뷰 중). 백엔드 구현은 먼저 진행해도 되지만, 실제 값 검증은 콘솔 배포 후에 하십시오.
>
> 배포 전 상태의 데이터는 전부 `action: "Shot"` / `"Pass"`이고 `baseAction`이 아예 없습니다. 집계 시 `baseAction`이 없으면 `action`을 그대로 상위 분류로 보는 폴백을 두면 옛 데이터도 슈팅·패스 카운트는 맞습니다(골·어시스트만 0). 백필은 §6.
...
**결과 콜백 인입 직후**(`XfpResultIngestService`가 클립·선수를 저장한 다음)에 그 `(teamId, playerRef)`들만 재계산하는 것을 권합니다.

- 콘솔에서 재전송할 때마다 즉시 반영됩니다 — 운영자가 태깅을 고치고 다시 보내면 랭킹도 따라옵니다.
- 대상이 그 신청에 등장한 선수로 한정돼 비용이 작습니다.
- 배치(야간 전량 재계산)로 해도 되지만, 그러면 재전송 후 최대 하루까지 랭킹이 옛 값으로 남습니다.

**프로필 행이 없는 선수**: `XfpPlayerProfileEntity`는 `(teamId, playerRef)` 키인데, 아직 프로필이 만들어지지 않은 선수(라인업에는 있으나 xFP 프로필 미생성)가 있을 수 있습니다. 이 경우 행을 새로 만들지, 집계를 건너뛸지는 백엔드 정책에 맡깁니다 — 다만 **건너뛰면 그 선수는 골을 넣어도 랭킹에 영원히 안 뜹니다.**
