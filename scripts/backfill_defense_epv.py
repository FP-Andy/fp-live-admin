...
3. 상태가치의 각도 항이 수비 전용 바닥(fpa.DEFENSE_CENTRALITY_FLOOR=0.70)을 쓰게
   됐다(2026-08-14). 공격 모델의 0.45 를 그대로 쓰던 시절엔 터치라인이 반 이하로
   눌려 측면 수비가 통째로 바닥을 쳤다(하프라인 측면 태클 58점). 이걸 안 돌리면
   기존 행의 측면 수비가 계속 옛 저평가 값으로 남는다.
