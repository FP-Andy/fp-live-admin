...
async function readResponse(response: Response) {
  ...
  if (!response.ok) throw new Error(typeof data.detail === 'string' ? data.detail : '팀 로고를 저장하지 못했습니다.');
  return data;
}
...
export default function TeamLogoLibrary({ competitions }: { competitions: Competition[] }) {
  ...
  useEffect(() => {
    if (competitions.length && !competitions.some(item => item.code === competition)) setCompetition(competitions[0].code);
  ...
  return <section className="card card-panel team-logo-library" aria-labelledby="team-logo-heading">
    ...
    <form className="team-logo-form" onSubmit={save}>
      <label className="field-stack"><span className="field-label">대회</span><select aria-label="팀 로고 대회" value={competition} onChange={event => { setCompetition(event.target.value); setTeam(''); }} disabled={busy}>
        {(competitions.length ? competitions : [{ code: 'K3', name: 'K3' }]).map(item => <option key={item.code} value={item.code}>{item.name} · {item.code}</option>)}
      </select></label>
      <label className="field-stack"><span className="field-label">팀명</span><input required maxLength={160} list="team-logo-team-options" placeholder="경기에 등록된 팀명" value={team} onChange={event => setTeam(event.target.value)} disabled={busy} />
        <datalist id="team-logo-team-options">{competitions.find(item => item.code === competition)?.team_options?.map(name => <option key={name} value={name} />)}</datalist>
      </label>
    ...
    <div className="team-logo-list" aria-live="polite">
      {loading ? <p className="muted">팀 로고를 불러오는 중…</p> : visible.length ? visible.map(logo => <article className="team-logo-item" key={logo.id}>
        ...
        <button type="button" className="btn-secondary" disabled={busy} onClick={() => { setTeam(logo.team_name); fileInput.current?.click(); }}>교체</button>
