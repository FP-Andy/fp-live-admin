...
export function fpaGraphic(match: Match, events: FpaEvent[], mode: MapMode, options: Options, normalize: boolean, scope: string) {
    ...
    events.forEach((e, i) => {
        ...
        const color = (mode === 'pass' || mode === 'kickin') ? (e.outcome === 'fail' ? '#ee6161' : e.outcome === 'success' ? '#3c8cff' : '#a5acb8') : mode === 'shot' ? (e.goal ? '#ff7400' : e.tags.includes('On Target') ? '#20bd92' : '#a5acb8') : eventColor(e);
    ...
    const legends = (mode === 'pass' || mode === 'kickin') ? '파랑: 성공 · 빨강 점선: 실패 · 회색: 결과 미기록' : mode === 'shot' ? '주황: 골 · 초록: 유효슈팅 · 회색: 기타 슈팅' : mode === 'sequence' ? '번호: 원본 이벤트 순서 · 각 이벤트의 실제 경로만 표시' : '파랑: 패스 · 주황: 슈팅 · 초록: 운반 · 보라: 수비 · 회색: 기타';
