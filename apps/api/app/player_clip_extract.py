...
끊김·재클릭을 줄이려 2단 방어:
  1) 트래커 설정(track_buffer↑·match_thresh↑)으로 놓친 트랙 수명을 늘려 재연결.
  2) stitch_player_tracks: 짧은 가림 구간을, 끊긴 트랙의 마지막 속도로 위치를
     외삽해 직후 재등장 트랙과 이어붙여 track_id 정규화(탐지 직후 1회).

ai-highlight/player_clip_extract.py 이식. 탐지는 highlight_pipeline._detect_objects
(2-pass 공 탐지·잔디마스크 포함)를 stride=3·player 트래커로 호출해 재사용한다.
"""
...
from .highlight_pipeline import _detect_objects

# 트랙 수명↑(끊김·재클릭↓)용 커스텀 추적 설정. 존재하는 첫 파일 사용.
# NOTE: 소스는 BoT-SORT+ReID(botsort_player.yaml)를 선호하나, ReID(`model: auto`)는
# 런타임 모델 자동 다운로드가 필요해 컨테이너 배포에서 취약하고 ultralytics 버전 의존성이
# 있다. 여기선 모션 기반 bytetrack_player.yaml 을 기본으로 둔다(외부 모델 불필요).
# ReID 가 검증되면 선호 순서를 botsort 우선으로 바꾸면 된다.
...
PLAYER_TRACKER = _first_existing("bytetrack_player.yaml", "botsort_player.yaml")
...
def run_player_detection(video_path: str, output_dir: str,
    ...
    progress_cb(pct, stage): 탐지가 0~90% (추적/공 패스), 트랙 정리·저장이 90~100%.
    """
    def _progress(payload: dict[str, Any]) -> None:
        if progress_cb:
            progress_cb(
                int(payload.get("percent") or 0),
                str(payload.get("detail") or payload.get("phase") or "탐지 중"),
            )

    det_df, fps = _detect_objects(
        video_path,
        yolo_model,
...
def _involvement_frames(det_df: pd.DataFrame,
    ...
    id 를 다른 선수에게 재사용하므로, 구간을 묶지 않으면 같은 id 의 다른 선수 장면까지 섞인다.
    """
...
def compute_player_segments(det_df: pd.DataFrame, fps: float,
    ...
    extract_player_clips 와 동일한 구간 산정 로직 — 추출 전 '예상 클립 수' 미리보기에 재사용.
