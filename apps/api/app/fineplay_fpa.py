...
def analysis_from_actions(
    ...
    *,
    ...
    team_view: dict[str, Any] = {
        ...
        **{k: primary[k] for k in ("xg", "xgot", "receptionXg", "epv", "pc") if primary.get(k) is not None},
    }
    if fpa_match_id:
                ...
                "actions": [
                    {k: v for k, v in a.items()
                     ...
                     if k in ("seq", "action", "baseAction", "actionLabel",
                              "xg", "xgot", "receptionXg", "epv", "pc", "startOffset", "endOffset")
                     and v is not None}
                    for a in player_actions
                ],
                **{k: best[k] for k in ("xg", "xgot", "receptionXg", "epv", "pc") if best.get(k) is not None},
            },
        }
